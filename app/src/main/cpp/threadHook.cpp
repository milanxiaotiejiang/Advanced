...
Java_com_example_advanced_thread_ThreadHook_enableThreadHookNative(JNIEnv *env, jclass type) {
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(env->FindClass("com/example/advanced/thread/ThreadHook")));
    if (kJavaClass == NULL)  {
