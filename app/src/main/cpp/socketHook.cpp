...
Java_com_example_advanced_network_SocketHook_enableSocketHookNative(JNIEnv *env, jclass type) {
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(env->FindClass("com/example/advanced/network/SocketHook")));
    if (kJavaClass == NULL)  {
